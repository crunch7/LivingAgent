...
class OllamaProvider(AIProviderInterface):
    """Ollama服务提供者（使用OpenAI库）"""
    def get_response(self, messages, callback_func):
        import openai
        
        model = self.config.get('ollamaModel', self.config.get('model', 'llama3'))
        url = self.config.get('ollamaUrl', 'http://localhost:11434/v1')
        ...
        try:
            ...
            client = openai.OpenAI(
                base_url=url,
                api_key="ollama",  # Ollama不需要API密钥，但OpenAI库需要
                max_retries=3,     # 自动重试
                timeout=30.0       # 超时设置
            )
            ...
            response = client.chat.completions.create(
                ...
                stream=True,
                temperature=self.config.get('temperature', AI.DEFAULT_TEMPERATURE),
                max_tokens=self.config.get('max_tokens', AI.DEFAULT_MAX_TOKENS),
                top_p=self.config.get('top_p', AI.DEFAULT_TOP_P)
            )
    ...
    def _get_dynamic_prompt(self):
        """获取动态系统提示词"""
        return (self.config.get('ai_instance', None).get_dynamic_system_prompt() 
                if self.config.get('ai_instance') 
                else AI.SYSTEM_PROMPT_TEMPLATE.format(
                    date=AI.DATE,
                    time=AI.TIME,
                    weekday=AI.WEEKDAY,
                    behavior="空闲"  # 默认行为
...
class ConfigManager:
    ...
    def get_default_config(self):
        ...
        return {
            ...
            # 服务特定配置
            'ollamaUrl': 'http://localhost:11434/v1',  # 更新为v1接口
            'zhipuApiKey': '',
            'deepseekApiKey': '',
            # 用户配置
            'username': ''  # 用户名默认为空
...
def _configure_ollama():
    ...
    url = input("请输入Ollama API URL (默认: http://localhost:11434/v1): ").strip()
    if not url:
